...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  expedia-cars-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  expedia-cars-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{".well-known", "getconfig", "gsi", "gsi-fedcm-config-passive-json", "gsi-fedcm-listaccounts", "maps", "maps-api-mapsjs-gen-204", "targeting-service", "typeahead",  }
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  expedia-cars-pp-cli workflow status

  # Show status as JSON
  expedia-cars-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("expedia-cars-pp-cli")
			}
