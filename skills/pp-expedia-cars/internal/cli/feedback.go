...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".expedia-cars-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXPEDIA_CARS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXPEDIA_CARS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "expedia-cars-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.expedia-cars-pp-cli/feedback.jsonl.
When ` + "`EXPEDIA_CARS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXPEDIA_CARS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  expedia-cars-pp-cli feedback list
  expedia-cars-pp-cli feedback list --limit 5
  expedia-cars-pp-cli feedback list --json`,
