...
In local mode: searches locally synced data only.`,
  ...
  expedia-cars-pp-cli search "error timeout"

  # Force local search only
  expedia-cars-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  expedia-cars-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  expedia-cars-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /ds/api/v1/toolkit/page.Car-Search/{id}/en_US/pwa
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/ds/api/v1/toolkit/page.Car-Search/{id}/en_US/pwa", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'expedia-cars-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
