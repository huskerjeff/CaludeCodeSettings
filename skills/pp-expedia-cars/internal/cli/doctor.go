...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  expedia-cars-pp-cli doctor
  expedia-cars-pp-cli doctor --json
  expedia-cars-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BROWSER_INTAKE_US5_DATADOGHQ_API_KEY=<your-key>"
				} else {
			...
			if os.Getenv("BROWSER_INTAKE_US5_DATADOGHQ_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "BROWSER_INTAKE_US5_DATADOGHQ_API_KEY")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BROWSER_INTAKE_US5_DATADOGHQ_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("expedia-cars-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'expedia-cars-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'expedia-cars-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'expedia-cars-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'expedia-cars-pp-cli sync' to refresh."
	}
	return report
}
