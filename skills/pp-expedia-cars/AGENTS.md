# Expedia Cars Printed CLI Agent Guide

This directory is a generated `expedia-cars-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
expedia-cars-pp-cli doctor --json
expedia-cars-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
expedia-cars-pp-cli which "<capability>" --json
expedia-cars-pp-cli <command> --help
```
...
```bash
expedia-cars-pp-cli <command> --agent
```
...
```bash
expedia-cars-pp-cli <command> --help
expedia-cars-pp-cli <command> --dry-run --agent
```
